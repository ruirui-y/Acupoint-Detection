...
class LoadImages:  # for inference
    ...
    def __next__(self):
        ...
        if self.video_flag[self.count]:
            ...
            # Read image
            self.count += 1
            img0 = cv2.imread(path)                                         # 用opencv读取图片
            assert img0 is not None, 'Image Not Found ' + path
            print(f'image {self.count}/{self.nf} {path}: ', end='')

        # 原始图片可能很大，把这行缩放成模型要求的尺寸640
        img = letterbox(img0, self.img_size, stride=self.stride)[0]

        # opencv读取的是BGR，模型要的是RGB 把图片的维度从[高度，宽度，通道] 变成 [通道，高度，宽度]
...
def load_image_rgb_ir(self, index):
    ...
    if (img_rgb is None) and (img_ir is None):  # not cached
        ...
        img_ir = cv2.imread(path_ir, cv2.IMREAD_GRAYSCALE)  # 强制读取为单通道 (H, W)
        ...
        if r != 1:  # if sizes are not equal
            ...
            img_ir = cv2.resize(img_ir, (int(w0 * r), int(h0 * r)),
                             interpolation=cv2.INTER_AREA if r < 1 and not self.augment else cv2.INTER_LINEAR)
            # 关键补充：cv2.resize 后如果是单通道会变成 (H, W)，需要补回轴变成 (H, W, 1)
            if img_ir.ndim == 2:
                img_ir = img_ir[:, :, None]
...
# 给图片缩放并增加黑边，使其符合模型要求的尺寸，选择缩放最狠的比例， 假如是1280*720，最后缩放成640*360
# 图片的上面填充140，下面填充140
def letterbox(img, new_shape=(640, 640), color=(114, 114, 114), auto=True, scaleFill=False, scaleup=True, stride=32):
    ...
    # 写死上面和下面填充140
    top = 140  # 上面填 140 像素
    bottom = 140  # 下面填 140 像素
    left = 0  # 左右不填（因为 1280 缩放 0.5 刚好是 640）
    right = 0

    img = cv2.copyMakeBorder(img, top, bottom, left, right, cv2.BORDER_CONSTANT, value=color)  # add border
    return img, ratio, (0, 140)
